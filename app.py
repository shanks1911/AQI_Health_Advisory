...
from logic.get_history import get_air_quality_history, get_extended_aqi_history
        ...
        if st.button("Generate 5-Day AQI Forecast", type="primary"):
            ...
            with st.spinner("Building comprehensive forecast..."):
                try:
                    ...
                    # ── OLD: Google-only, hard-capped at 30 days ──────────────
                    # days_total, days_recent_gap = 30, 2
                    # end_date_hist = (datetime.now() - timedelta(days=days_recent_gap)).strftime('%Y-%m-%d')
                    # start_date_hist = (datetime.now() - timedelta(days=days_total)).strftime('%Y-%m-%d')
                    # aqi_history_data = get_air_quality_history(lat, lon, days_back=days_total)
 
                    # ── NEW: extended fetch — Google (30 days) + OpenAQ (120 days)
                    # days_total drives both the AQI fetch AND the weather date range.
                    # days_recent_gap is the overlap window fetched via get_recent_weather()
                    # instead of the archive API (OpenMeteo archive has a 5-day lag).
                    days_total, days_recent_gap = 150, 2
                    end_date_hist = (datetime.now() - timedelta(days=days_recent_gap)).strftime('%Y-%m-%d')
                    start_date_hist = (datetime.now() - timedelta(days=days_total)).strftime('%Y-%m-%d')
                    aqi_history_data = get_extended_aqi_history(lat, lon, days_back=days_total)
...
if 'current_data' in st.session_state:
    ...
    with tab1:
        ...
        # ── Model Performance (CHANGED — Issue 2 + Issue 4) ──────────────────
        if 'training_results' in st.session_state:
            st.divider()
            st.header("AI Model Performance")
            results = st.session_state['training_results']

            # ── OLD: single-model metrics only ────────────────────────────────
            ...
            # ── NEW: best-model headline + full comparison table ──────────────
            # Headline metrics for the winning model (same 4 numbers as before,
            # just relabelled so the context is clear)
            col1, col2, col3, col4 = st.columns(4)
            with col1:
            ...
            with col2:
                st.metric("R² Score", f"{results['r2']:.3f}")
            with col3:
                st.metric("MAE (AQI pts)", f"{results['mae']:.2f}")
            with col4:
                st.metric("Variance Explained", f"{results['r2'] * 100:.1f}%")

            # ── NEW: all-models comparison table (Issue 2) ────────────────────
            # train_model.py now returns results['all_models'] — a list of
            # {name, r2, mae} dicts for every model that was trained.
            # SARIMA / LSTM results can be appended here after running
            # logic/train_baseline.py offline.
            if results.get('all_models'):
                st.subheader("Model Comparison")
                st.caption(
                    "All models trained on the same 80/20 time-series split. "
                    "Higher R² and lower MAE = better. "
                    "SARIMA and LSTM rows are added after running logic/train_baseline.py."
                )

                comparison_df = pd.DataFrame(results['all_models'])
                comparison_df.columns = ['Model', 'R²', 'MAE (AQI pts)']
                comparison_df = comparison_df.sort_values('R²', ascending=False).reset_index(drop=True)

                # Highlight the winning row in light green
                def _highlight_best(row):
                    if row['Model'] == results['model_name']:
            ...
            # ── NEW: evaluation charts (Issue 4) ─────────────────────────────
            # train_model.py now saves 3 charts under evaluation_charts/ and
            # returns their paths in results['chart_paths'].
            chart_paths = results.get('chart_paths', {})
            if chart_paths:
                st.subheader("Evaluation Charts")
                st.caption(
                    "Generated at 300 DPI — suitable for direct use in the paper. "
                    "Files saved to evaluation_charts/ in the project root."
                )
                chart_col1, chart_col2, chart_col3 = st.columns(3)

                with chart_col1:
                    p = chart_paths.get('predicted_vs_actual')
                    if p and os.path.exists(p):
                        st.image(p, caption="Fig 1 — Predicted vs Actual AQI", use_column_width=True)
                    else:
                        st.info("Chart not yet generated.")

                with chart_col2:
                    p = chart_paths.get('residual_distribution')
                    if p and os.path.exists(p):
                        st.image(p, caption="Fig 2 — Residual Distribution (all models)", use_column_width=True)
                    else:
                        st.info("Chart not yet generated.")

                with chart_col3:
                    p = chart_paths.get('forecast_degradation')
                    if p and os.path.exists(p):
                        st.image(p, caption="Fig 3 — Forecast Accuracy Degradation", use_column_width=True)
                    else:
                        st.info("Chart not yet generated.")
